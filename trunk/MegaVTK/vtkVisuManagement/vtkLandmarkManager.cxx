/*=========================================================================
  ...
  Program:   vtkINRIA3D
  Module:    $Id: vtkLandmarkManager.cxx 938 2008-09-09 13:23:19Z ntoussaint $
  Language:  C++
  Author:    $Author: ntoussaint $
  Date:      $Date: 2008-09-09 09:23:19 -0400 (Tue, 09 Sep 2008) $
...
vtkCxxRevisionMacro(vtkLandmarkManager, "$Revision: 938 $");
vtkStandardNewMacro(vtkLandmark);
vtkCxxRevisionMacro(vtkLandmark, "$Revision: 938 $");
...
void vtkLandmarkManager::ReadFile (const char* filename)
{
  ...
  if (strcmp(header, "LANDMARK MANAGER FILE 1.4") != 0)
  {
    buffer.close();
...
vtkLandmark* vtkLandmarkManager::CreateAndAddLandmark (void)
{
  ...
  vtkViewImage* view = vtkViewImage::SafeDownCast (this->ViewList->GetItemAsObject(this->GetViewIdToTrust()));
  const double* pos = view->GetCurrentPoint();
  landmark->SetPosition(view->GetCurrentPoint());
