...
Program:   vtkINRIA3D
Module:    $Id: vtkFibersManager.cxx 1009 2008-11-19 18:06:36Z filus $
Language:  C++
Author:    $Author: filus $
Date:      $Date: 2008-11-19 13:06:36 -0500 (Wed, 19 Nov 2008) $
...
#ifdef MegaVTK_USE_HWSHADING
#include "vtkFiberMapper.h"
//using namespace bmia;
#endif

vtkCxxRevisionMacro(vtkFibersManager, "$Revision: 1009 $");
...
//   vtkBoxWidget *widget = reinterpret_cast<vtkBoxWidget*>(caller);
  ...
  this->FiberLimiter->SetVOI ((int)xmin, (int)xmax, (int)ymin, (int)ymax, (int)zmin, (int)zmax);
  ...
  while ( test!=0 )
  {
    if( rnpts == npts )
    {
      ...
      if( i==npts && pto[i-1]==rpto[i-1])
      {
        ...
        for( int i=0; i<npts; i++)
        {
          rpto[i]=rpto[0]; // collapse the fiber on itself
        }
...
void vtkFibersManager::RemoveAllActors()
{
  if( m_RWin )
  {
    ...
    vtkRenderer* first_renderer = m_RWin->FindPokedRenderer(m_RWin->GetLastEventPosition()[0],
							    m_RWin->GetLastEventPosition()[1]);
    if (first_renderer)
    {
      first_renderer->RemoveActor ( m_Actor );
      first_renderer->RemoveActor ( m_PickerCallback->GetPickedActor() );
      first_renderer->RemoveActor ( m_HelpMessage );
    }
  }
}
...
void vtkFibersManager::SetInput(vtkPolyData* input)
{
  ...
  {
    m_RWin->GetRenderWindow()->GetRenderers()->InitTraversal();
    vtkRenderer* first_renderer = m_RWin->FindPokedRenderer(
				m_RWin->GetLastEventPosition()[0],
				m_RWin->GetLastEventPosition()[1]);

    if (first_renderer)
    {
      first_renderer->AddActor ( m_Actor );
      first_renderer->AddActor ( m_PickerCallback->GetPickedActor() );
      first_renderer->AddActor ( m_HelpMessage );
    }
